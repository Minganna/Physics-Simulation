...
bool MovingSphereToPlaneCollision(const glm::vec3& n, const glm::vec3& c0, const glm::vec3& c1, const glm::vec3& q, float r, glm::vec3& ci)
{
	...
	if (glm::abs(d0) <= r)
	{
		...
		//Set the time of collision to zero and set the position of the contact sphere as the sphere position
		ci = c0;
		t = 0.0f;
		return true;
	}
	if (d0 > r&&d1 < r)
	{
		t = (d0 - r) / (d0 - d1); //normalize time
		ci = (1 - t)*c0 + t * c1;
		return true;
	}

	return false; // else sphere is moving away from and parellel plane
